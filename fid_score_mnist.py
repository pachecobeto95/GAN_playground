...
class ImagePathDataset_PIL(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        if self.transforms is not None:
            img = self.transforms(self.img[i])
        return img
...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu', resize=False, img_size=128):
    ...
    if files != "mnist-eval" and batch_size > len(files):
    ...
    if (resize):
      ...
      dataset = ImagePathDataset_PIL(files, transforms=transformation)

    # Pegando o data set de teste em batches
    # FALTA: GERAR AS IMAGENS EM BATCHES
    if(files == "mnist-eval"):
